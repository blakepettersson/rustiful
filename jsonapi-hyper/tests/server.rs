...
impl Service for TestService {
    ...
    fn call(&self, req: Request) -> Self::Future {
        ...
        match (req.method(), req.path()) {
            (&hyper::Method::Get, "/params") => {
                println!("{}", req.path());
                let query = req.query().expect("nooo").as_bytes();
                println!("{}", req.query().expect("noooo"));
                tx.lock().unwrap().send(Msg::Chunk(query.to_vec())).unwrap();
...
#[test]
fn server_get_query_string_response() {
