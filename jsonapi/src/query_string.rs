...
pub trait QueryString<'a> {
    ...
    type FilterField;
    type Params: Default +
        TypedParams<Self::SortField, Self::FilterField> +
    ...
    fn from_str(query_string: &'a str) -> Result<Self::Params, QueryStringParseError> {
        ...
        for param in query_string.split('&') {
            ...
            match key_value_pair {
                (Some(""), None) | (None, None) => {},
                (Some(key), None) => {
                    return Err(QueryStringParseError::InvalidValue(format!("Invalid param: {}", key)))
                },
                (None, Some(value)) => {
                ...
                (Some(key), Some(value)) if key == "sort" => {
                    if !params.sort().is_empty() {
                        ...
                        return Err(QueryStringParseError::InvalidValue(format!("Duplicate sort key: {}", value)))
                    }
